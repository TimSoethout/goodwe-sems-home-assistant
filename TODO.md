...
- [x] Keep sanitized legacy and SEMS+ response examples in
  [`api_examples/`](api_examples/).
- [x] Use captured, sanitized API responses as fixtures for integration
  regression tests where practical.
- [ ] Add fixtures for newly observed device types, response codes, and
  optional fields before expanding normalization or entity support.
- [ ] Review debug captures for secrets and identifiers before sharing or
  committing them.

## SEMS+ migration follow-up

- [x] Map active SEMS+ inverter status code `5` to `Normal` (#219).
- [x] Map SEMS+ three-phase telemetry factors
...
- [x] Accept supported non-`INVERTER` device types such as
...
- [x] Use each discovered device's type for telemetry and telecounting instead
