"""Tests for SEMS number entities (Home Assistant integration-style)."""
...
MOCK_GET_BATTERY_IMMEDIATE_CHARGING_STATES = {
    "sn": "GW0000SN000TEST1",
    "data": {"47545": 0, "47546": 75, "47603": 65},
    "resultValues": {
        "GW0000SN000TEST1": {
            "47545": [{"value": 0}],
            "47546": [{"value": 75}],
...
async def test_immediate_charging_number_states_from_coordinator(
    hass: HomeAssistant,
    enable_custom_integrations: None,
) -> None:
    """Test that the numbers are created and have the expected state."""
    ...
    entity_id = ent_reg.async_get_entity_id(
        Platform.NUMBER, DOMAIN, "GW0000SN000TEST1-mppt1_battery-end_charge_soc"
    )
    ...
    entity_id = ent_reg.async_get_entity_id(
        Platform.NUMBER,
        DOMAIN,
        "GW0000SN000TEST1-mppt1_battery-bat_immediate_charge_power",
    )
...
async def test_adjust_immediate_charging_numbers(
    hass: HomeAssistant,
    enable_custom_integrations: None,
) -> None:
    """Test that the numbers update correctly when the immediate charging states change."""
    del enable_custom_integrations
    entry = MockConfigEntry(
        domain=DOMAIN,
        title="Test",
    ...
    hass.states.async_set(entity_id, "70.0")
    await hass.async_block_till_done()
    state = hass.states.get(entity_id)
    assert state is not None
...
async def test_immediate_charging_numbers_not_created_if_immediate_charging_not_supported(
    hass: HomeAssistant,
    enable_custom_integrations: None,
) -> None:
